...
def fetch_qr_code_data(qr_code_id, access_token, created_date):
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"Data for QR Code {qr_code_id} fetched successfully!")
...
def save_to_csv(parsed_data, qr_code_id, title, output_folder):
    if parsed_data or not parsed_data:  # Create file even if no data
        ...
        print(f"Data for QR Code {qr_code_id} has been successfully saved to '{output_filename}'")
    else:
...
if __name__ == "__main__":
    ...
    csv_filename = input("Enter the CSV file name (full path, e.g., /Users/user1/qrcg-api-statistics/QRCG STATISTICS EXPORTS/qrcg_statistics_12345.csv): ").strip()

    # Define the folder where all CSV files will be saved
    output_folder = "GRANULAR STATISTICS EXPORTS"  # You can customize this folder name
